use plotipus::ui::app::ResizeEdge;
use plotipus::ui::input::{ClickAction, KeyAction};
use plotipus::ui::{PlotipusApp, PlotipusEvent};
...
struct App {
    ...
    plotipus_app: Option<PlotipusApp>,
    target_frame_duration_ms: u64,
    #[allow(dead_code)]
    event_proxy: EventLoopProxy<PlotipusEvent>,
}

impl ApplicationHandler<PlotipusEvent> for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let app = PlotipusApp::new(&window);
        window.request_redraw();
        self.window = Some(window);
        self.plotipus_app = Some(app);
    }
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::Resized(size) => {
                if let (Some(app), Some(window)) = (&mut self.plotipus_app, &self.window) {
                    app.resize(size);
                    window.request_redraw();
                }
            }
            ...
            WindowEvent::RedrawRequested => {
                if let Some(app) = &mut self.plotipus_app {
                    app.render();
                }
            }
            ...
            WindowEvent::CursorLeft { .. } => {
                if let Some(app) = &mut self.plotipus_app {
                    app.hovered_button = plotipus::ui::app::HoveredButton::None;
            ...
            WindowEvent::MouseInput { state, button, .. } => {
                let (Some(app), Some(window)) = (&mut self.plotipus_app, &self.window) else {
                    return;
                };
...
fn main() {
    let event_loop = EventLoop::<PlotipusEvent>::with_user_event().build().unwrap();
